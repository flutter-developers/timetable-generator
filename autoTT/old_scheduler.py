...
class Timetable:
    ...
    @classmethod
    def create_genome(cls, data, psuedo_data):
        global batch_courses, course_faculty, course_duration, day
        temp_timetable = []
        for _ in psuedo_data:
            t = list(_.course)[0].frequency
            d = list(_.course)[0].duration
            for j in range(t):
                g = Gene(_.batch, _.course, _.faculty, random.choice(day), 6, _.room)
                temp_timetable.append(g)
        for _ in data:
            t = list(_.course)[0].frequency
            d = list(_.course)[0].duration
            for j in range(t):
...
def initiation():
    ...
    for _ in range(0, len(ll), 2):
        ...
        courselist.append(t)
    courselist.append(Course('DUMMY', 'DUMMY', 'DUMMY', 1, 3))
    ...
    ps = [Data({batchlist[0]}, {courselist[9]}, set(), None, 'A103'),
          Data({batchlist[1]}, {courselist[9]}, set(), None, 'B414'), ]
    data = [
        Data({batchlist[0], batchlist[1]}, {courselist[0]}, set(), None, ""),
    ...
    print(data, len(data))
    return batchlist, professorlist, courselist, data, ps


def main():
    batchlist, professorlist, courselist, data, psuedo_data = initiation()
    ...
    for _ in range(POPULATION_SIZE):
        gnome = Timetable.create_genome(data, psuedo_data)
